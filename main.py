...
def plot_result():
    ...
    for curve_number in range(N_CURVES):
        ...
        eligible_f1_values = [f1 for index, f1 in enumerate(f1_values) if viable_solution[index]]
        eligible_f2_values = [f2 for index, f2 in enumerate(f2_values) if viable_solution[index]]
        non_viable_f1.extend([f1 for index, f1 in enumerate(f1_values) if not viable_solution[index]])
        non_viable_f2.extend([f2 for index, f2 in enumerate(f2_values) if not viable_solution[index]])
